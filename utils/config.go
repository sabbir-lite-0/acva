...
func LoadConfig(filename string) (Config, error) {
	...
	// Load Gemini API keys from environment if enabled
	if config.Gemini.Enabled {
		if len(config.Gemini.APIKeys) == 0 {
			// Try to load from environment variables
			for i := 1; i <= 5; i++ {
				envVar := fmt.Sprintf("GEMINI_API_KEY_%d", i)
				if apiKey := os.Getenv(envVar); apiKey != "" {
					config.Gemini.APIKeys = append(config.Gemini.APIKeys, apiKey)
				}
			}
			...
			if len(config.Gemini.APIKeys) == 0 {
				if apiKey := os.Getenv("GEMINI_API_KEY"); apiKey != "" {
					...
					return config, fmt.Errorf("Gemini enabled but no API keys found")
				}
			}
		}
	}
